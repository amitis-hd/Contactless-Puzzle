...
def recognize_voice_commands(stop_event , restart_event, voice_command):
...
# Notes: 
def game_setup(stop_event , restart_event, voice_command):
    ...
    while not stop_event.is_set():
        ...
        game_area = pygame.Rect(0, 0, screen_width * 0.75, screen_height)#75% of the game for the image
        message_area = pygame.Rect(screen_width * 0.75, 0, screen_width * 0.25, screen_height)  #25% for voice message display
        ...
        # set up the image
        image = load_and_scale_image(image_path, game_area.width, game_area.height)
            ...
            for event in pygame.event.get():
                ...
                if complete:
                    ...
                    if count == 2: #two pieces have been selected to be swapped
                        ...
                        pygame.display.flip()
                        indecies = []
            if voice_command.Value:
                voice_command = font.render(voice_command.value, True, (255, 255, 255))
                
                # Calculate the position for the text in the message area
                text_x = message_area.x + (message_area.width - voice_command.get_width()) // 2
                text_y = message_area.y + (message_area.height - voice_command.get_height()) // 2


                screen.blit(voice_command, (text_x, text_y)) #display text

                #Clear the command after displaying
                voice_command.value = ''
...
if __name__ == "__main__":
    ...
    #for voice recognition save
    manager = Manager()
    voice_command = manager.Value('c', '')

    cursor_process = Process(target=find_and_move_to_face, args=(stop_event, ))
    image_process = Process(target=game_setup, args=(stop_event, restart_event, voice_command ))
    voice_process = Process(target=recognize_voice_commands, args=(stop_event, restart_event, voice_command ))
